...
// El cuerpo de una peticion a Vercel no puede pasar de 4.5 MB — medido: hasta
// 4.0 MB pasa, 4.4 MB devuelve FUNCTION_PAYLOAD_TOO_LARGE. Base64 infla un
// tercio. Asi que las imagenes se reducen aqui hasta que el total quepa, y si
// no cabe no se manda: rebotar con 413 no le sirve a nadie.
const PRESUPUESTO = 3200000;                                   // base64, total
const PASOS = [[1568, 0.85], [1100, 0.8], [800, 0.72], [560, 0.6]];

async function aBitmap(blob){
  try{ return await createImageBitmap(blob, {imageOrientation:'from-image'}); }
  catch(e){ try{ return await createImageBitmap(blob); }catch(e2){ return null; } }
}
function b64de(buf){
...
async function codificar(bmp, lado, calidad){
  const k=Math.min(1, lado/Math.max(bmp.width,bmp.height));
  const w=Math.max(1,Math.round(bmp.width*k)), h=Math.max(1,Math.round(bmp.height*k));
  const c=document.createElement('canvas'); c.width=w; c.height=h;
  c.getContext('2d').drawImage(bmp,0,0,w,h);
  return await new Promise(r=>c.toBlob(r,'image/jpeg',calidad));
}
// Devuelve { imagenes, avisos }. Nunca devuelve algo que no quepa.
async function prepararImagenes(archivos){
  const avisos=[], bmps=[];
  for(const f of archivos||[]){
    const bm = await aBitmap(f);
    if(!bm){ avisos.push(`"${f.name}": el navegador no puede leer este formato. Si es HEIC (el de iPhone y Mac por defecto), ábrela y expórtala como JPG.`); continue; }
    bmps.push({bm, nombre:f.name});
  }
  if(!bmps.length) return {imagenes:[], avisos};
  let blobs=null;
  for(const [lado,cal] of PASOS){
    blobs = [];
    for(const x of bmps) blobs.push(await codificar(x.bm, lado, cal));
    if(blobs.reduce((t,b)=>t+b.size,0)*4/3 <= PRESUPUESTO) break;
  }
  // Ni en el paso mas chico caben todas: se mandan las que quepan y se avisa.
  const usados=[]; let total=0;
  for(let i=0;i<blobs.length;i++){
    const peso = blobs[i].size*4/3;
    if(total+peso > PRESUPUESTO){
      avisos.push(`"${bmps[i].nombre}" no se envió: entre todas superan el límite de 4.5 MB del servidor. Manda menos imágenes por vez.`);
      continue;
    }
    total+=peso; usados.push(blobs[i]);
  }
  const imagenes=[];
  for(const b of usados) imagenes.push({media_type:'image/jpeg', data:b64de(await b.arrayBuffer())});
  for(const x of bmps) if(x.bm.close) x.bm.close();
...
function sampleDeServidor(){
  const f = async (input, o={}) => {
    ...
    if(o.images && o.images.length){
      const prep = await prepararImagenes([...o.images]);
      if(prep.imagenes.length) cuerpo.images = prep.imagenes;
      if(prep.avisos.length){
        if(!prep.imagenes.length) throw {code:'image_rejected', message:prep.avisos.join(' ')};
        S.error = prep.avisos.join(' ');
      }
    }
    const crudo = JSON.stringify(cuerpo);
    if(crudo.length > 4000000) throw {code:'prompt_too_large', message:'La petición pesa '+(crudo.length/1048576).toFixed(1)+' MB y el servidor acepta 4.5 MB. Quita alguna imagen o acorta el brief.'};
    let r;
    try{ r = await fetch('/api/sample',{method:'POST',headers:cabeceras(),body:crudo,signal:o.signal}); }
    ...
    if(r.status===413) throw {code:'prompt_too_large', message:'Las imágenes pesan demasiado para el servidor. Recarga la página y vuelve a adjuntarlas: la versión nueva las reduce automáticamente.'};
...
function motivoSinClaude(){
  if(SRV && !SRV.ia) return 'El servidor no tiene OPENAI_API_KEY configurada. Ponla en las variables de entorno del proyecto de Vercel y vuelve a desplegar.';
...
// ------------------------------------------------------------ llamadas
function errMsg(e){
  const m = {not_granted:'No autorizaste el uso del modelo en esta página.', rate_limited:'Límite de uso alcanzado. Espera un momento y vuelve a intentar.', prompt_too_large:'El brief o el contexto es demasiado grande para una llamada.', invalid_json:'La respuesta no vino en el formato esperado. Vuelve a intentar.', refused:'El modelo rechazó esta entrada. Cambia el brief.', cancelled:'Detenido.', images_unavailable:'Esta vista del visor no puede enviar imágenes al modelo. Para trabajar con referencias visuales abre la app en https://ai-production-director.vercel.app', image_rejected:'Imagen rechazada: tipo o tamaño no válido.', sampling_disabled:'El modelo no está disponible en esta cuenta.'};
  // Cuando el mensaje lo escribio esta pagina (nombra el archivo, el peso, el
  // formato), gana sobre el texto generico del codigo.
  if(e && e.message && ['image_rejected','prompt_too_large','upstream_error','not_granted'].includes(e.code) && e.message.length>40) return e.message;
...
function vBloqueo(m){
  m.innerHTML = `<div class="hd"><div><h2>Contraseña</h2><p class="sub">Esta app usa la cuenta de OpenAI del servidor. La contraseña evita que la gaste alguien más.</p></div></div>
...
(async()=>{
  ...
  if(!sample){
    ...
    if(SRV && SRV.ia){ sample = sampleDeServidor(); if(SRV.clave && !SRV.claveOk) S.bloqueo=true; }
  }
  ...
  const parts=[ sample?(SRV?('ChatGPT '+(SRV.modelo||'')+' vía servidor propio'):'Claude listo'):(SRV&&!SRV.ia?'Falta OPENAI_API_KEY en el servidor':'Modelo no disponible'), db?'guardado en la nube':'guardado local', mcp?'Higgsfield listo':'video por cuadros' ];
