...
// El cuerpo de una peticion a Vercel no puede pasar de 4.5 MB, y base64 infla
// un tercio: una foto de camara sale rebotada con 413. Se reduce a ~1.2
// megapixeles, que es a lo que la API la baja de todos modos, asi que no se
// pierde nada util.
const MAX_PX = 1200000, MAX_LADO = 1568, YA_CHICA = 900000;
async function reducir(blob){
  ...
  catch(e){ try{ bmp = await createImageBitmap(blob); }catch(e2){ return blob; } }
  let w=bmp.width, h=bmp.height;
  const escala = Math.min(1, Math.sqrt(MAX_PX/(w*h)), MAX_LADO/Math.max(w,h));
  if(escala>=1 && blob.size<YA_CHICA){ if(bmp.close) bmp.close(); return blob; }
  w=Math.max(1,Math.round(w*escala)); h=Math.max(1,Math.round(h*escala));
  const c=document.createElement('canvas'); c.width=w; c.height=h;
  c.getContext('2d').drawImage(bmp,0,0,w,h);
  if(bmp.close) bmp.close();
  const chica = await new Promise(r=>c.toBlob(r,'image/jpeg',0.85));
  return (chica && chica.size<blob.size) ? chica : blob;
}
async function aBase64(blob){
  const b2 = await reducir(blob);
  const buf=await b2.arrayBuffer(); let bin=''; const b=new Uint8Array(buf);
  for(let i=0;i<b.length;i+=0x8000) bin+=String.fromCharCode.apply(null,b.subarray(i,i+0x8000));
  return {media_type: b2.type||'image/jpeg', data: btoa(bin)};
}
...
function sampleDeServidor(){
  const f = async (input, o={}) => {
    ...
    if(crudo.length > 4200000) throw {code:'prompt_too_large', message:'La petición pesa '+Math.round(crudo.length/1048576)+' MB y el servidor acepta 4.5 MB. Quita alguna imagen o acorta el brief.'};
    let r;
